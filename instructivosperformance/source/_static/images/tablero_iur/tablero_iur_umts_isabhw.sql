...
-- Actualizacion: Monica Pellegrini. Fecha:30.03.2016. Motivo: Se cambia la logica agrendo el igual para que tome el limite.

VARIABLE MR_NCPA           VARCHAR2(50);
VARIABLE MR_NCAC           VARCHAR2(50);
VARIABLE V_FECHA_DESDE     VARCHAR2(20);
VARIABLE MASCARA           VARCHAR2(20);
VARIABLE SEMANA            VARCHAR2(5);
VARIABLE MR_SIEVET         VARCHAR2(50);

EXEC :MR_NCPA              := 'NO COMERCIAL PROC ALTA';
EXEC :MR_NCAC              := 'NO COMERCIAL ACEPT CALIDAD';
EXEC :V_FECHA_DESDE        := '&1';
EXEC :MASCARA              := 'DD.MM.YYYY';
EXEC :SEMANA               := 'D-S';
EXEC :MR_SIEVET            := 'SITIOS EVENTUALES'



BEGIN

DELETE FROM TABLERO_UMTS_IUR_ISABHW WHERE FECHA = TO_DATE(:V_FECHA_DESDE, :MASCARA);
...
SELECT SUM(T1.RAB_CSS_VOZ_DEN)                                                  RAB_CSS_VOZ_DEN,
 ...
 WHERE T1.INT_ID = T3.INT_ID (+)
   ...
   AND N1.ESTADO_MR NOT IN (:MR_NCPA , :MR_NCAC, :MR_SIEVET)
   AND OB.WCELL_VALID_FINISH_DATE > SYSDATE
   AND T1.FECHA = TO_DATE(:V_FECHA_DESDE, :MASCARA)
   AND SEMANA = :SEMANA
...
SELECT /*+ ORDERED */
 ...
 WHERE T1.INT_ID = O1.WCELL_ID
   AND T1.ESTADO_MR NOT IN (:MR_NCPA , :MR_NCAC, :MR_SIEVET)
   AND T1.FECHA = TO_DATE(:V_FECHA_DESDE, :MASCARA)
   AND  WCELL_VALID_FINISH_DATE > SYSDATE
...
GROUP BY T1.FECHA,WCELL_NAME,
 ...
 WHERE INDICADOR_ID = 2040
   ...
   AND  WCELL_VALID_FINISH_DATE > SYSDATE
   AND O1.WCELL_NAME IS NOT NULL
   AND FECHA = TO_DATE(:V_FECHA_DESDE, :MASCARA)
        )D
