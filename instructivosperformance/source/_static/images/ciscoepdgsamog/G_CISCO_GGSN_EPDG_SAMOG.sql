...
PROCEDURE P_CISCO_GGSN_EPDG_ISABHW (P_FECHA_DESDE IN CHAR);
...
PROCEDURE P_CISCO_GGSN_SAMOG_ISABHW (P_FECHA_DESDE IN CHAR);
  ...
  FROM (SELECT T1.FECHA,
       ...
       WHERE T1.FECHA = T2.FECHA
         ...
         AND T1.FECHA BETWEEN TO_DATE(FECHA_DESDE, 'DD.MM.YYYY HH24')
                          AND TO_DATE(FECHA_HASTA, 'DD.MM.YYYY HH24') + 1/24 + 59/1440
         ) T1
      ...
      WHEN DML_ERRORS THEN
        L_ERRORS := SQL%BULK_EXCEPTIONS.COUNT;
                ...
                LOOP
                    ...
                    PKG_ERROR_LOG_NEW.P_LOG_ERROR('P_CISCO_GGSN_EPDG_DAY',L_ERRNO,L_MSG,
                    'V_BULK_ERRORS EXCEPTION FECHA: '||
                    TO_CHAR(T_ARRAY_COL(L_IDX).FECHA)||
                    ' SERVID '||TO_CHAR(T_ARRAY_COL(L_IDX).SERVID));
      ...
      -- CAPTURE EXCEPTIONS TO PERFORM OPERATIONS DML
           L_ERRORS := SQL%BULK_EXCEPTIONS.COUNT;
              ...
              LOOP
                  ...
                  PKG_ERROR_LOG_NEW.P_LOG_ERROR('P_CISCO_GGSN_EPDG_BH',L_ERRNO,L_MSG,
                    'V_BULK_ERRORS EXCEPTION FECHA: '||
                    TO_CHAR(T_ARRAY_COL(L_IDX).FECHA)||
                    ' SERVID '||TO_CHAR(T_ARRAY_COL(L_IDX).SERVID));
...
BEGIN
  P_CISCO_GGSN_EPDG_REWORK_HOUR(P_FECHA_DESDE, P_FECHA_HASTA);
  P_CISCO_GGSN_SAMOG_REWORK_HOUR(P_FECHA_DESDE, P_FECHA_HASTA);
END;
...
BEGIN
  P_CISCO_GGSN_EPDG_REWORK_DAY(P_FECHA_DESDE, P_FECHA_HASTA);
  P_CISCO_GGSN_EPDG_REWORK_BH(P_FECHA_DESDE, P_FECHA_HASTA);
  
  P_CISCO_GGSN_SAMOG_REWORK_DAY(P_FECHA_DESDE, P_FECHA_HASTA);
  P_CISCO_GGSN_SAMOG_REWORK_BH(P_FECHA_DESDE, P_FECHA_HASTA);
...
BEGIN
  P_CISCO_GGSN_EPDG_REWORK_WEEK(P_FECHA_DESDE, P_FECHA_HASTA);
  P_CISCO_GGSN_SAMOG_REWORK_WEEK(P_FECHA_DESDE, P_FECHA_HASTA);
END;
...
BEGIN
  G_PARTITION_MGMT.P_TRUNCATE_PARTITION_WEEK('CISCO_GGSN_EPDG_ISABHW',P_FECHA_DESDE, P_FECHA_HASTA);
  FOR TR IN FECHAS LOOP
    P_CISCO_GGSN_EPDG_ISABHW(TR.FECHA);
  END LOOP;
...
BEGIN
  ...
  G_PARTITION_MGMT.P_TRUNCATE_PARTITION_WEEK('CISCO_GGSN_SAMOG_ISABHW',P_FECHA_DESDE, P_FECHA_HASTA);
  FOR TR IN FECHAS LOOP
    P_CISCO_GGSN_SAMOG_ISABHW(TR.FECHA);
  END LOOP;
